...
{
	 $("#main").html("");
	$.post("selectXzdm.action",(issearch?searchjson:json),function(data,textStatus)
	{
		var totalrow=0;
		for(var i in data)
		{
		...
		$("#main").append('<tr style="display:none;">'+
							...
							'<td id="season" style="word-break:break-all">'+data[i].season+'</td>'+
							'<td id="zhenqu" style="word-break:break-all">'+data[i].zhenqu+'</td>'+
							...
							'<td id="rjy" style="word-break:break-all">'+data[i].rjy+'</td>'+
							'<td id="dabiao" style="word-break:break-all">'+data[i].dabiao+'</td>'+
							'</tr>');
							totalrow++;
		}
...
function search(){
	...
	var json="{";
		...
		json+="\"season\":\""+$("#searchseason").val()+"\","; 
		json+="\"zhenqu\":\""+$("#searchzhenqu").val()+"\","; 
		json+="\"ad\":\""+$("#searchad").val()+"\","; 
		json+="\"zl\":\""+$("#searchzl").val()+"\","; 
		json+="\"gmsy\":\""+$("#searchgmsy").val()+"\","; 
		json+="\"rjy\":\""+$("#searchrjy").val()+"\","; 
		json+="\"dabiao\":\""+$("#searchdabiao").val()+"\","; 
 ...
 <script type="text/javascript">
	...
	function appendline(){
		...
		"<tr>"+
			...
			"<td id='season'><input type='text' style='width:100%; height:100%;'/></td>"+
			"<td id='zhenqu'><input type='text' style='width:100%; height:100%;'/></td>"+
			...
			"<td id='rjy'><input type='text' style='width:100%; height:100%;'/></td>"+
			"<td id='dabiao'><input type='text' style='width:100%; height:100%;'/></td>"+
		"</tr>"
		);
	...
	function save(funsave) 
	{
		...
		$this.children().each(function() {
	        ...
	        $.post("updateXzdm.action",getRseData(),function(data,textStatus){
			reload("",function(){
			last="save";
		 	allop+="save;";	
		 	if(data!=1)
	...
	function remove() {
	 ...
	 $.post("deleteXzdm.action",{ id : selected },function(data,textStatus){
		isedit=false;
		selectedrow=-1;
		if(data!=1)
			alert(data);
	 ...
	 allop+="delete;";
        ...
        function getRseData() {
			    var jsonStr = "{"; // json格式开始
					...
					jsonStr+="\"season\":\""+$this.find("#season").html()+"\",";
					jsonStr+="\"zhenqu\":\""+$this.find("#zhenqu").html()+"\",";
					jsonStr+="\"ad\":\""+$this.find("#ad").html()+"\",";
					jsonStr+="\"zl\":\""+$this.find("#zl").html()+"\",";
					jsonStr+="\"gmsy\":\""+$this.find("#gmsy").html()+"\",";
					jsonStr+="\"rjy\":\""+$this.find("#rjy").html()+"\",";
					jsonStr+="\"dabiao\":\""+$this.find("#dabiao").html()+"\",";
...
<body>
	...
	<div id="mainwindow">	
	<div id="tou" style="">镇区水功能考核断面数据管理</div>
		<h5>查询条件：</h5>
		...
		<div class="input-group" style="margin-bottom: 10px">
  		  ...
  		  <span class="input-group-addon" id="sizing-addon2">季度</span><input id="searchseason" type="text" class="form-control" placeholder="season" aria-describedby="sizing-addon2">
  		  <span class="input-group-addon" id="sizing-addon2">镇区</span><input id="searchzhenqu" type="text" class="form-control" placeholder="zhenqu" aria-describedby="sizing-addon2">
  		  ...
  		  <input id="searchrjy" type="hidden" class="form-control" placeholder="rjy" aria-describedby="sizing-addon2">
  		  <input id="searchdabiao" type="hidden" class="form-control" placeholder="dabiao" aria-describedby="sizing-addon2">
		</div>
		...
		<div id="tools">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<!-- 模板下载 -->
			<a href="download.action?filepath=镇区水功能考核断面导入模板.xlsx"><span class="glyphicon glyphicon-save icon"/></a>
	...
	<table id="table" class="table table-bordered "><!-- table-hover -->
		...
		<div>
			...
			<div style="margin-top: 30px; margin-left: 25px; width: 250px;">
				...
				<sf:form id="fileupload" method="post" enctype="multipart/form-data" action="inputxzdm.action">
			        <%--这里设置文件上传--%>
			        本地文件:
			        <input type="file" id="file" name="file"/>
				 </sf:form>
			        <input onclick="inputxzdm()" style="margin-top: 10px" type="button" class="btn btn-info" value="导入"/>
